...
class ClassificationTask(BaseTask):
    ...
    def prepare_inputs(self, dataset: Any, 
        ...
        if label_column and label_column in dataset.column_names:
            ...
            labels = [self.label_map.get(label) for label in labels]
    ...
    def parse_output(self, raw_output: str) -> TaskOutput:
        ...
        # Map to known labels
        best_match = None
        # Sort keys by length descending to match longer labels first
        sorted_labels = sorted(self.label_map.keys(), key=lambda x: len(str(x)), reverse=True)
